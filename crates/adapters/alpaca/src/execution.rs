...
impl ExecutionClient for AlpacaExecutionClient {
    ...
    fn submit_order(&self, cmd: SubmitOrder) -> anyhow::Result<()> {
        ...
        // A price-less order (market / market-to-limit / trailing) carries no `reference_price`
        // from Python (the pyo3 factory can't set it), so fall back to a cached decision-time
        // price (quote mid, else last trade) — otherwise value-based risk rules fail-closed and
        // deny every market-type order. Reducing-exposure exits bypass valuation regardless.
        let reference_price = order
            .reference_price()
            .or_else(|| self.reference_price_for(instrument_id));
